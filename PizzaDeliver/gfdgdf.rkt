...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname gfdgdf) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;;
;; *****************************************************
;;   Heli Wang (20552080)
;;   CS 135 Fall 2014
...
(define (accommodate-one-top? i-tree-in-onetop mytop)
            (cond [(empty? i-tree-in-onetop) false]
                  [(equal? (i-node-ingredient i-tree-in-onetop) mytop)
                   (cond 
                     [(>= (i-node-quantity i-tree-in-onetop) 1.5)
...
(check-expect (accommodate? sample-i-tree med-bt-pizza3) true)
sample-i-tree
(accommodate-one-top? sample-i-tree "tomatoes")